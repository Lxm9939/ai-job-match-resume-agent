...
读取用户配置的公开岗位源，在 robots.txt 允许时低频访问公开页面，并将各来源的成功、跳过或失败状态结构化记录。默认来源选项包括常见招聘平台和公司官网 Careers，但不会因为平台名称一刀切拒绝。
...
它把网络访问风险隔离在独立模块中。一个来源失败只会产生可解释状态，不会中断 V1、V2 或其他岗位源，同时每条岗位都能回到原始链接核查。若遇到登录、验证码、HTTP 429 或 robots 不允许，会跳过并建议用户改用 CSV/Excel 导入或手动粘贴 JD。
