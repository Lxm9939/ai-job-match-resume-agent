...
读取用户配置的公开岗位源，在 robots.txt 允许时低频访问公开 Careers 页面，并将各来源的成功、跳过或失败状态结构化记录。
...
- `List[CrawlResult]`
- 每个结果包含来源、岗位列表、跳过原因、错误信息和抓取数量

**示例输出**

```json
{
  "source": {"source_name": "Example Company Careers", "allowed": true},
  "jobs": [
    {
      "job_title": "AI Product Intern",
      "city": "北京",
      "source_url": "https://company.example/careers/ai-product-intern"
    }
  ],
  "skipped_reason": "",
  "error_message": "",
...
它把网络访问风险隔离在独立模块中。一个来源失败只会产生可解释状态，不会中断 V1、V2 或其他岗位源，同时每条岗位都能回到原始链接核查。
