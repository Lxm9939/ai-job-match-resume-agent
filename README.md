...
- 目标方向输入：支持常用岗位方向下拉选择，也支持填写更具体的自定义目标岗位方向。
...
- 支持填写目标岗位方向、城市、岗位类型和公司偏好；目标方向可从常用选项选择，也可输入自定义岗位方向。
- 复用 V1 Agent 对每个岗位执行证据匹配、关键词覆盖和五维评分。
- 按匹配总分降序生成岗位排行榜和分档投递建议。
- 为每个岗位生成针对性简历优化、投递话术和面试准备清单。
- 支持下载批量 Markdown 匹配报告。
- 岗位链接会经过规范化和占位识别；示例数据、空链接或 `example.com` 不会被伪装成可点击链接。

### V3：公开岗位源自动抓取

- 从用户配置或自动生成的公开岗位搜索页 / Careers 静态 HTML 页面读取岗位候选。
- 默认提供 Boss 直聘、智联招聘、猎聘、前程无忧、拉勾、实习僧、牛客招聘、应届生求职网、LinkedIn Jobs、Indeed、Seek、GradConnection、公司官网 Careers 和自定义公开 URL 作为来源选项。
- Boss 直聘、猎聘、LinkedIn Jobs、Indeed、Seek 可根据关键词和城市生成公开搜索 URL；智联招聘、前程无忧、拉勾、实习僧、牛客招聘、应届生求职网、GradConnection 和公司官网 Careers 需要用户提供具体公开 URL。
- 请求前检查 robots.txt，网络检查失败时默认跳过。
- 不因为平台名称一刀切拒绝；是否进入解析由 robots.txt、HTTP 状态、页面内容和是否需要登录/验证码决定。
- 解析器按 JSON-LD / Next.js / 页面 JSON、通用岗位卡片、中文岗位文本切片多层 fallback 提取岗位；JS 动态渲染页会给出明确替代方案。
- 对 `source_url` 进行相对链接补全；无效、占位或 Demo 链接会标记说明，不渲染为可点击链接。
...
- 来源状态：记录 `public_accessible`、`robots_disallowed`、`login_required`、`captcha_or_blocked`、`http_error`、`parse_failed`、`no_public_url`、`demo_data` 等状态。
...
- 系统默认提供常见招聘平台和公司官网 Careers 作为岗位来源选项。对于配置了公开搜索模板的平台，系统会根据关键词和城市生成搜索 URL；对于未配置稳定模板的平台，用户可在自定义公开 URL 中提供具体页面。系统仅解析公开可访问页面，不进行登录、验证码、Cookie、代理池或浏览器指纹绕过。
- 如果 Boss 直聘、智联招聘、猎聘、前程无忧等页面需要登录、验证码、访问受限或 robots.txt 不允许，系统会跳过并显示原因，建议改用 CSV/Excel 导入或手动粘贴 JD。
- 系统不会因为平台名称一刀切拒绝，是否可抓取由 robots.txt、HTTP 状态、页面内容和解析结果决定。
- Demo/sample 数据中的岗位链接不代表真实岗位；无效链接会被标记，而不是伪装成可点击链接。
- 对公开页面中的相对链接，如 `/jobs/123`，系统会基于列表页 URL 自动补全。

当前通用解析器以公开静态 HTML 为主，会依次尝试结构化数据、岗位卡片和中文文本切片。站点结构不同可能导致字段不完整，此时系统会保留来源链接，并将过短内容标记为“JD 信息不足”。如果页面由 JavaScript 动态渲染且静态 HTML 中没有岗位文本，系统会提示改用 CSV/Excel 导入、手动粘贴 JD，或提供岗位详情页 URL。
...
- V3 的通用 HTML 解析依赖页面结构，动态渲染、登录后可见或结构特殊的网站可能无法提取完整岗位，但会尽量给出可解释的失败原因。
- V3 只访问明确配置且 robots.txt 允许的公开页面；不会进行登录绕过、验证码绕过、Cookie 复用、代理池或浏览器指纹绕过。
- 常见招聘平台在 README 和页面中作为来源选项出现，不代表一定能公开抓取成功；无法公开访问时会被跳过并记录原因。
