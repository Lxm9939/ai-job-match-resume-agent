...
- 系统默认提供常见招聘平台和公司官网 Careers 作为岗位来源选项，但仅在页面公开可访问且 robots.txt 允许时进行轻量抓取；不会进行登录绕过、验证码绕过、Cookie 复用、代理池或浏览器指纹绕过。
...
- V3 的通用 HTML 解析依赖页面结构，动态渲染、登录后可见或结构特殊的网站可能无法提取完整岗位。
